...
BEGIN
  ...
  IF (:varCounter = 0) THEN
     varStatement := '';
  ELSE
     varStatement := 'CREATE JWT PROVIDER JWTPROVIDER_B282B026_D5CF_4A92_B235_2A4BDDFBBFDE WITH ISSUER ''http://p2001081147trial.localhost:8080/uaa/oauth/token'' CLAIM ''user_name'' AS EXTERNAL IDENTITY';
     EXEC :varStatement;
  END IF;
END;
--
...
BEGIN
  ...
  IF (:varCounter = 0) THEN
     varStatement := '';
  ELSE
     varStatement := 'ALTER JWT PROVIDER JWTPROVIDER_B282B026_D5CF_4A92_B235_2A4BDDFBBFDE SET ISSUER ''http://p2001081147trial.localhost:8080/uaa/oauth/token'' CLAIM ''user_name'' AS EXTERNAL IDENTITY';
     EXEC :varStatement;
  END IF;
END;
--
